...
HEADER_ = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:131.0) Gecko/20100101 Firefox/131.0'
...
def parse_results_amz(query, page_num=None, verbose=True):
    ...
    for product in products[:NUM_PROD_LIMIT]:
        ...
        price_div = product.find("div", {'data-cy': ['secondary-offer-recipe', 'price-recipe']})
        price = price_div.find(text=re.compile(r'\$')) if price_div is not None else None
        

        result = {
            'asin': asin,
            'Title': title.text.strip(),
            'Price': price.text.strip().strip("$") if price is not None else "N/A"
        }
...
def parse_item_page_amz(asin, verbose=True):
    ...
    # Features
    try:
        features_div = soup.find(name="div", attrs={"id": "productFactsDesktopExpander"})
        features = [feature.text for feature in features_div.find_all(name="span", class_="a-color-base")]
    except AttributeError:
        features = "N/A"
    product_dict["BulletPoints"] = list(features)
    ...
    try:
        ...
        option_selectors = option_body.findAll(name="select")
        for block in option_selectors:
            opt_list = []
            for opt in block.findAll("option")[1:]:
                opt_list.append(opt.text)
            options[block["data-a-touch-header"]] = opt_list
    except AttributeError as e:
        options = {}
