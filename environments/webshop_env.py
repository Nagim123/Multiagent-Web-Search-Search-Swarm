# COPIED AND MODIFIED FROM WEBSHOP SOURCE CODE
...
class WebAgentSiteEnv(gym.Env):
    ...
    def __init__(self, observation_mode='html', **kwargs):
        ...
        service = Service(join(dirname(abspath(__file__)), 'chromedriver.exe'))
        options = Options()
        if 'render' not in kwargs or not kwargs['render']:
            options.add_argument("--headless=old")  # don't show browser
